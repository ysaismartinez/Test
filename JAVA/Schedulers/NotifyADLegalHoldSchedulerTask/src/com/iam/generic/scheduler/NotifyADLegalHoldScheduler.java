...
public class NotifyADLegalHoldScheduler extends TaskSupport{
	...
	public void execute(HashMap hashMap) throws Exception {
		...
		try{
			...
			String searchFilter = ((String)hashMap.get("Filter String"));
			...
			for(int i=0;i<lookupArr.length;i++){
				...
				if(failedHashMap.size()>0){
					...
					while(itr.hasNext())
					{
						...
						logger.info(className+"Inside Lookup"+temp);
						//if( temp.contains("Organization Name Updated"))
						if( temp.contains(searchFilter))
						{
							logger.info(className+"Match Found");
							sendMail=1;
							attrList.add(temp);
						}
					}
			...
			{
				logger.info(className + methodName + "No Events to send in Mail");
			}
